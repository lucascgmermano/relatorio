...
Durante o ano de 2022 foram notificados `r sum(dd22$casos, na.rm=T)` casos e **`r sum(dd22$obitos)`** óbitos, o que correspondeu à taxa de incidência de **`r round(sum(dd22$casos)/830499*100000, 0)`** casos por 100 mil habitantes e à taxa de letalidade de **`r round(sum(dd22$obitos)/sum(dd22$casos)*100, 0)`%** entre os doentes. A seguir são apresentados os dados sobre a ocorrência dos casos e óbitos, distribuição espacial do Covid-19 e as coberturas vacinais detalhadas:
...
```{r, out.width= "100%"}
 ff  %>% 
      ...
      ggplot(aes(x = semana, y = percentual))+
        ...
        geom_text(aes(label=percentual), size=2.4, vjust=1.5, col='white')+
...
ff  %>% 
        ...
        geom_text(aes(label=percentual), size=3, vjust=1.5, col='white')+
        scale_x_continuous(n.breaks = max(ff[ff$ano==2022]$semana))+
        labs(y="Percentual do total de casos", x="Semana epidemiológica")+
        facet_wrap( ~ faixa, ncol = 3, scales = 'free')+
        theme_bw()+
...
ggplot()+
  ...
  # geom_smooth(aes(x = data, y = cob.d1*7, col='Cob.d1'),se = F, method = 'gam')+
  # geom_smooth(aes(x = data, y = cob.d2*7, col='Cob.d2'),se = F, method = 'gam')+
  # geom_smooth(aes(x = data, y = cob.d.adic*7, col='Cob.D.Adic'),se = F, method = 'gam')+
  ...
  geom_text(hjust=-.5, aes(x = data, y = casos, label = casos, angle=90), size=3)+
  scale_x_date(date_breaks = "1 month", date_labels = "%b")+
  scale_y_continuous(n.breaks = 10, expand = expansion(mult = c(0,.25)))+
  # scale_y_continuous(sec.axis = sec_axis(~./7, name = "Cobertura (%)", breaks = seq(0,100,5)))+
