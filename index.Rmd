---
title: "Covid-19: Casos e óbitos <Br> Painel 2025"
...
dd25 <- read_csv2(here('dados','dados2025.csv')) %>% 
  mutate(
...
```
<Br>
Casos:  **`r as.integer(sum(dd25$casos, na.rm=T))`** <Br>
Óbitos: **`r sum(dd25$obitos)`** <Br>
Taxa de incidência: **`r as.integer(round(sum(dd25$casos)/830499*100000, 0))`** casos por 100 mil habitantes. <Br>
Taxa de letalidade: **`r round(sum(dd25$obitos)/sum(dd25$casos)*100, 1)`%** entre os doentes. <Br>
...
acumulado %>% 
kableExtra::kable(col.names = c("N.", "2020","2021","2022","2023","2024","2025","Total")) %>% 
...
ggplot(quadro, aes(x = mesL, y = ano, fill = casos)) +
  geom_tile() +
  scale_fill_gradient(low = "#aed6f1", high = "blue4") +
  scale_y_continuous(breaks = seq(min(quadro$ano), max(quadro$ano))) +
  theme_minimal() +
...
#Gráfico
filter(casos, lubridate::year(data)==2025) %>% 
  ggplot()+
