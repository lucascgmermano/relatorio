---
title: "Covid-19 e coberturas vacinais: <Br> Painel 2022"
...
ddvac <- read_csv2(here('dados','ddvac.csv')) %>% 
  filter(between(data, as.Date("2021-01-01"), as.Date("2022-12-31")) &
           dose != "biv")
```
...
dd22  %>% 
  ...
  scale_x_continuous(breaks = seq(min(dd22$semana), max(dd22$semana), by = 3))+
...
#Gráfico
filter(casos, lubridate::year(data)==2022) %>% 
  ggplot()+
  ...
  ggforce::facet_zoom(xy =  data >= as.Date("2022-12-11") &
                        data <= as.Date("2022-12-31"), horizontal = F, shrink = F)+
  theme_minimal()+
  ...
  print(
    left_join(tabyl(subset(ddvac, muni==i), cat.pop.etaria, dose),
      ...
      mutate(cob.d1=round(d1/pop.etaria*100, 1),
             ...
             cob.ref3=round(dadic3/pop.etaria*100, 1),
             # cob.ref4=round(ref4/pop.etaria*100, 1),
             muni = NULL) %>%
      select(cat.pop.etaria, pop.etaria, d1, d2, dadic1, dadic2, dadic3, 
...
tab <- cbind.data.frame(n.mun, pop.vacgeral, doses.geral[,c("d1","d2","dadic1","dadic2")])
tab <- tab %>% rbind.data.frame(c("Total", as.numeric(colSums(tab[2:6]))))
tab <- cbind.data.frame(tab[1], sapply(X = tab[2:6], FUN = as.numeric))

cbind.data.frame(tab, 
                 cobd1 = round(tab["d1"]/tab$pop.vacgeral*100,1),
                 cobd2 = round(tab["d2"]/tab$pop.vacgeral*100,1),
                 cob.dadic1 = round(tab["dadic1"]/tab$pop.vacgeral*100,1),
                 cob.dadic2 = round(tab["dadic2"]/tab$pop.vacgeral*100,1)) %>% 
...
tab <- cbind.data.frame(pop.geraletaria, doses.geralidade[,2:5]) # monta a tabela
...
tab <- cbind.data.frame(doses.comorb[1],pop.comorb,doses.comorb[,2:5]) # monta a tabela c/ pop
tab <- tab %>% rbind.data.frame(c("Total", as.numeric(colSums(tab[2:6])))) # adiciona totais
tab <- cbind.data.frame(tab[1], sapply(X = tab[,2:6], FUN = as.numeric)) # transforma numerico
...
#Gráfico
ggplot(graf[year(graf$data) %in% c('2021','2022'),])+
...
ddvac %>% 
  ...
  scale_x_discrete(limits = format(seq.Date(from = as.Date("2021-01-01"), 
                                            to = as.Date("2022-12-31"), by = "month"),format="%b/%y"))+
