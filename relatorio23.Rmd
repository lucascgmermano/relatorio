---
title: "Covid-19 e coberturas vacinais: <Br> Painel 2023"
...
dd23  %>% 
  ...
  scale_x_continuous(breaks = seq(min(dd23$semana), max(dd23$semana), by = 2))+
...
for (i in x) {
  ...
  mutate(cob.d1=round(d1/pop.etaria*100, 1),
         ...
         cob.ref1=round(dadic1/pop.etaria*100, 1),
         cob.ref2=round(dadic2/pop.etaria*100, 1),
         cob.biv=round(biv/pop.etaria*100, 1),
         muni = NULL) %>%
  select(cat.pop.etaria, pop.etaria, 
         d1, d2, dadic1, dadic2, 
         biv,
         cob.d1, cob.d2, cob.ref1,
         cob.ref2, 
         cob.biv) %>%
  kable(col.names = c("Faixa etária", "População", "D1", 
                      "D2", "Ref1", "Ref2","Biv",
                      "D1%", "D2%", "Ref1%", "Ref2%","Biv%"), caption = i) %>%
  kable_styling(full_width = F,
                bootstrap_options = c("striped", "hover",
                                      "condensed", "responsive")) 
    )
...
tab <- cbind.data.frame(n.mun, pop.vacgeral, doses.geral[,2:7])
tab <- tab %>% rbind.data.frame(c("Total", as.numeric(colSums(tab[2:8]))))
tab <- cbind.data.frame(tab[1], sapply(X = tab[2:8], FUN = as.numeric))

cbind.data.frame(tab[,c("n.mun", "pop.vacgeral","d1","d2","dadic1","dadic2","biv")], 
                 cobd1 = round(tab["d1"]/tab$pop.vacgeral*100,1),
                 cobd2 = round(tab["d2"]/tab$pop.vacgeral*100,1),
                 cob.ref1 = round(tab["dadic1"]/tab$pop.vacgeral*100,1),
                 cob.ref2 = round(tab["dadic2"]/tab$pop.vacgeral*100,1),
                 cob.biv = round(tab["biv"]/tab$pop.vacgeral*100,1)) %>% 
  kable(col.names = c("Município", "População", "D1","D2","Ref1",
                      "Ref2","Biv","D1%","D2%","Ref1%","Ref2%","Biv%")) %>% 
  kable_styling(full_width = F, 
                bootstrap_options = c("striped", "hover", 
                                      "condensed", "responsive"))
rm(doses.geral,tab)
...
tab <- cbind.data.frame(n.mun, pop.ped[,3], doses.ped[,3:5])
...
cbind.data.frame(tab, 
                 ...
                 cob.ref1=round(tab[,"dadic1"]/tab[,"pop5a11anos"]*100,1)) %>% 
...
cbind.data.frame(tab,
                 ...
                 cob.dadic1=round(tab[,5]/tab[,2]*100,1),
                 cob.dadic2=round(tab[,6]/tab[,2]*100,1),
                 cob.biv=round(tab[,7]/tab[,2]*100,1)) %>% 
  kable(col.names = c("Faixa etária", "População", "D1","D2","Ref1","Ref2",
                      "Biv","D1%","D2%","Ref1%","Ref2%","Biv%")) %>%                  
  kable_styling(full_width = F, 
                bootstrap_options = c("striped", "hover", 
                                      "condensed", "responsive"))
...
tab <- cbind.data.frame(doses.gest[1],pop.gest,doses.gest[,2:6]) # monta a tabela c/ pop
tab <- tab %>% rbind.data.frame(c("Total", as.numeric(colSums(tab[2:7])))) # adiciona totais
tab <- cbind.data.frame(tab[1], sapply(X = tab[,2:7], FUN = as.numeric)) %>%  # transforma numerico
  select(-biv,biv)
...
cbind.data.frame(tab,
                 ...
                 cob.dadic1=round(tab[5]/tab[2]*100,1),
                 cob.dadic2=round(tab[6]/tab[2]*100,1),
                 cob.dadic2=round(tab[7]/tab[2]*100,1)) %>% 
...
doses.data <- tabyl(ddvac, data, dose, show_na = F) %>%
  select(-dadic3,-dadic4,-dadic5,-biv,biv) %>% 
  setNames(c('data','d1','d2','ref1','ref2','biv'))
...
graf <- cbind(casos.doses,
              ...
              round(cumsum(x = casos.doses$biv)/766509*100, 1)) %>% 
  setNames(c("data","casos","mmovel","d1","d2","ref1","ref2","biv",
             'cob.d1','cob.d2','cob.ref1','cob.ref2','cob.biv'))
...
ggplot(graf[year(graf$data)!='2020',])+
  ...
  geom_line(aes(x = data, y = cob.biv*20, col='Cob.biv'))+
...
ddvac %>%
  filter(between(data, as.Date("2021-01-01"), Sys.Date()) &
           dose %in% c("d1","d2","dadic1","dadic2","biv"))%>% 
  group_by(data, dose) %>% 
  summarise(num = n()) %>% 
  mutate(dose = factor(dose, levels = c("d1","d2","dadic1","dadic2","biv"))) %>% 
  ...
  scale_fill_manual(values = RColorBrewer::brewer.pal(n = 5, name = 'Paired'), 
                    labels = c("D1","D2","Reforço 1","Reforço 2","Bivalente"))+
...
<p style="text-align: center;">Seguir para:</p>
  <form style="text-align: center;">
    ...
    <input type="button" value="2024" onclick="window.location.href='index.html'">
  </form>
</body>
</html>