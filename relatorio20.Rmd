...
Este relatório tem o objetivo de caracterizar a evolução temporal dos casos e óbitos por Covid-19.
...
considerados as fontes de dados oficiais para avalização epidemiológica do agravo durante durante a pandemia. Os resultados foram agrupados por data, município, distribuição geográfica e grupos etários. São apresentadas frequências absolutas dos `casos`, `óbitos`, `taxa de incidência` e `taxa de letalidade` ([*ver definições*](formulas.html)).
<Br>
...
Durante o ano de 2020 foram notificados **`r sum(dd20$casos)/1000`** casos e **`r sum(dd20$obitos)`** óbitos, o que correspondeu à taxa de incidência de **`r round(sum(dd20$casos)/830499*100000, 0)`** casos por 100 mil habitantes e à taxa de letalidade de **`r round(sum(dd20$obitos)/sum(dd20$casos)*100, 0)`%** entre os doentes. A seguir são apresentados os dados sobre a ocorrência dos casos,óbitos e distribuição espacial:
<Br>
...
graf <- casos %>%
  setNames(c('data', 'casos', 'mmovel'))

#Gráfico
ggplot(graf,aes(x = data, y = mmovel))+
